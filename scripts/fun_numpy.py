...
def alert(df, parameters=None):
    import numpy as np
    logger.debug("fun alert")
    logger.debug(df)
    logger.debug("get rows that match")
    logger.debug( df.query('run_status > 0 and run_status < 5'))
    data = np.where(df.query('run_status > 0 and run_status  < 5'), "Online", "Offline")
...
def main(args):
    ...
    #  Create numpy dimensional array of rpm and pressure to  separate df for scoring then merge the two DFs
    val = alert(df=df, parameters=None)
